...
#On normalize notre dataset avec la moyenne et l’écart-type utilisé pour les images du dataset d’ImageNet ;
# la normalisation étant autrement trop couteuse (computation intensive).
# De nombreux papiers montrent que ces valeurs marchent bien pour un grand nombre de problèmes de classification.
...
image_directory = "/Users/pauleo/Desktop/Raw_Dataset_700Imgs" # à adapter en fonction de l'endroit où sont stockées les données
dataset_full = datasets.ImageFolder(image_directory, data_transforms)
loader_full = torch.utils.data.DataLoader(dataset_full, batch_size=100, shuffle=True, num_workers=2)
...
loader_train = torch.utils.data.DataLoader(dataset_train, batch_size=120, shuffle=True, num_workers=2)
...
# on indique qu'il est inutile de calculer les gradients des paramètres de resnet (on "freeze" les poids)
for param in resnet.parameters():
    param.requires_grad = False

# on remplace la dernière couche fully connected à 1000 sorties (classes d'ImageNet) par une fully connected à 4 sorties (nos classes).
# par défaut, les gradients des paramètres cette couche seront bien calculés
resnet.fc = nn.Linear(in_features=resnet.fc.in_features, out_features=4, bias=True)
...
# fonction d'entraînement modèle
def train_model(model, loader, optimizer, criterion, n_epochs=3):
...
resnet.train(True)
torch.manual_seed(42)
train_model(resnet, loader_train, optimizer, criterion, n_epochs=1)

# on définit une fonction d'évaluation
def evaluate(model, dataset):
    avg_loss = 0.
    avg_accuracy = 0
    loader = torch.utils.data.DataLoader(dataset, batch_size=103, shuffle=False, num_workers=2)
    for data in loader:
        inputs, labels = data
